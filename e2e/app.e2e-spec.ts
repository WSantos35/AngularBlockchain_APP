...
describe('Starting tests for tienditawebPrueba7', function() {
  ...
  it('website title should be tienditawebPrueba7', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tienditawebPrueba7');
    })
  });

  it('network-name should be tiendita_prueba3@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tiendita_prueba3@0.0.1.bna');
    });
  });

  it('navbar-brand should be tienditawebPrueba7',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tienditawebPrueba7');
    });
  });
